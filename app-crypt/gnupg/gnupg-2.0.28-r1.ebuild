...
src_prepare() {
	...
	epatch "${FILESDIR}/${PN}-2.0.28-m4-pth-config-flags-path.patch"
	epatch "${FILESDIR}/${PN}-2.0.28-configure-pth-config-flags-path.patch"
	epatch_user
}
...
src_configure() {
	...
	econf \
		...
		--enable-large-secmem \
		--without-adns \
		--includedir=/build/peppy/usr/include \
		--oldincludedir=/build/peppy/usr/include \
		--with-pth-prefix=/build/peppy/usr \
		"${myconf[@]}" \
		$(use_enable bzip2) \
		$(use_enable nls) \
